...
function NumericInput({ name, value, onChange }) {
  return (
    <div className="input">
      <label>{formatSettingName(name)}</label>
      <input type="number" value={value} onChange={(evt) => onChange(name, Number(evt.target.value))} />
    </div>
  );
}
  ...
  return (
    <div className="control-panel">
      ...
      <div className="source-link">
        <a href="https://github.com/goong-io/goong-map-react/tree/main/examples/interaction" target="_new">
          View Code ↗
        </a>
      </div>
      <hr />
